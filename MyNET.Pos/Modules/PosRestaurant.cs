...
namespace MyNET.Pos
{
    public partial class PosRestaurant : Form
    {
        #region  Members
        ...
        protected decimal bonusCardAmountUsed = 0M;
        ...
        public static List<Partner> allPartners = Partner.Search("Status=0").ToList();
        ...
        //payment dialog form
        private frmPayment mPaymentDialog;
            ...
            try
            {
                ...
                if (cbPartners.SelectedItem != null)
                {
                    if (ug.Rows.Count > 0)
                    {
                        ...
                        ////Screeni i pageses per me pa klienti
                        ...
                        if (Convert.ToDecimal(txtTotalSum.Text) > 0 || Globals.Settings.FiscalPrinterType == "Tremol" || bonuscard!=null)
                        {

                            bonuscardValue1 = bonuscardValue;
                            ...
                            if (dialogResult == DialogResult.OK)
                            {
                                ...
                                if (rows > 0)
                                {
                                    ...
                                    if (savedPOSId > 0)
                                    {
                                        SavedPOS.DeleteSavedPOS(savedPOSId.ToString());
                                    ...
                                    if (bonuscard != null)
                                    {
                                        ...
                                        {
                                            decimal totalPoints = mTotalSum + bonuscard.TotalAmount;
                                            //decimal currentPoints = bonuscard.CurrentAmount == 0 ? totalPoints - (bonuscardValue / bonuscard.PointsToEur) : (bonuscard.CurrentPointsValue - bonuscardValue) / bonuscard.PointsToEur;
                                            decimal currentPoints = bonuscard.CurrentAmount + mTotalSum - (bonuscardValue/bonuscard.TotalPoints/bonuscard.PointsToEur);
                                            decimal totalpointsValue = Math.Round(totalPoints * bonuscard.TotalPoints * bonuscard.PointsToEur, 2);
                                            decimal currentpointsValue = Math.Round(currentPoints * bonuscard.TotalPoints * bonuscard.PointsToEur, 2);

                                            BonusCard.UpdateTotalAmountBonusCard(totalPoints, bonuscard.Id);
                                            BonusCard.UpdateCurrentAmountBonusCard(currentPoints, bonuscard.Id);
                                            BonusCard.UpdateTotalPointsValueBonusCard(totalpointsValue, bonuscard.Id);
                                            BonusCard.UpdateCurrentPointsValueBonusCard(currentpointsValue, bonuscard.Id);
        ...
        {
            List<ItemsDiscount> item1 = Item.GetAllItemSS();

            //var allItems = Item.GetItemWithName("");
            // Shuffle the items to introduce randomness
            var shuffledItems = item1.OrderBy(x => Guid.NewGuid());
            Random random = new Random();
            // Take the first 5 items from the shuffled list
            int randomNumber = (int)(random.NextDouble() * (20 - 1) + 1);
            var randomItems = shuffledItems.Take(randomNumber);
            ...
            {
                row.Cells[12].Value = Convert.ToDecimal(randomNumber);
                SimulateCellEndEdit(row.Index, 12);

            }
            //if (ug.Rows.Count > 0)
            //{
            //    btnDirectPay_Click(null, null);
        ...
        private void SavePayment()
        {
            ...
            if (mPaymentDialog.TotalCash > 0)
            {
                ...
                pmt.ClientCash = mPaymentDialog.clientPayed;
                payments.Add(pmt);
            }
            if (mPaymentDialog.numPos1.Text != "")
            {
                if (mPaymentDialog.BankPos1 != 0 && Convert.ToDecimal(mPaymentDialog.numPos1.Text) != 0.0M)
                {
                    ...
                    posP1.ClientCash = mPaymentDialog.clientPayed;
                    payments.Add(posP1);
                }
            }
        ...
        private void PosSales_Load(object sender, EventArgs e)
        {
            ...
            if (globals.BarcMode == 1)
            {
                ...
                txtsearchB.Width = 600;
                txtB.Width = 580;
                txtsearchB.Location = new System.Drawing.Point((this.Width - 600) / 2, 0);
                txtB.Location = new System.Drawing.Point((this.Width - 600) / 2, 0);
            ...
            mAllItems = sett.BarcMode == 0 ? Services.Item.GetAllItem().ToList() : null;
            ...
            lblFiscalCount.Text = daily.DailyFiscalCount.ToString();

            txtShortcuts.Text = "F1 - Çmimi, F2 - Sasia , F3 - Zbritja ne Artikull, F4 - Zbritja Totale % , F5 - Zbritja Totale me vlerë, F6 - Detajet e Artikujve, F7- Shtyp Kuponin, F8- Pages Direkte, F9 - Konverto , F11 - FullScreen";
        ...
        private void CallCellEndEditForColumn12()
        {
            ...
            Random random = new Random();
            decimal cashPayment = (decimal)(Convert.ToDouble(mTotalSum));

            //// Create the cash payment object
            Services.Payment pmt = new Services.Payment();
            pmt.AmountPaid = cashPayment;
            payments.Add(pmt);
            ...
            if (mSaleId == 0)
            {
                ...
                if (Sale.getAllSales().Count > 0)
                {
                    if (Sale.getAllSales().Where(p => p.PosId == Globals.Station.Id).Count() > 0)
                    {
                        sale.SaleId = Sale.getAllSales().Where(p => p.PosId == Globals.Station.Id).Last().SaleId + 1;
                    }
                    else
                    {
        ...
        private void AdjustText()
        {
            ...
            if (Screen.PrimaryScreen.WorkingArea.Width == 1920f)
            {
                ...
                word_cancel_order.Font = new Font("Lato", 22, FontStyle.Regular);
                word_print_the_coupon.Font = new Font("Lato", 22, FontStyle.Regular);
                ...
                catButtonL = 10;
                txtsearchB.Font = new Font("Microsoft Sans Serif", 14);
                txtB.Font = new Font("Microsoft Sans Serif", 18);
            ...
            if (x >= 1400 && x <= 1680)
            {
                ...
                CbSize = new Size(100, 50); subCbSize = new Size(90, 40);
                catButtonL = 10;
                txtsearchB.Font = new Font("Microsoft Sans Serif", 12);
                txtB.Font = new Font("Microsoft Sans Serif", 16);
            ...
            if (x >= 1280 && x <= 1366)
            {
                ...
                catButtonL = 0;
                txtsearchB.Font = new Font("Microsoft Sans Serif", 10);
                txtB.Font = new Font("Microsoft Sans Serif", 14);
        ...
        private void Pos_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.F1)
            {
                if (ug.Rows.Count > 0)
                {
                    ug.Columns[20].SortMode = DataGridViewColumnSortMode.NotSortable;
                    ug.CurrentCell = ug[20, ug.CurrentRow.Index];
                    ug.BeginEdit(true);
                }
            ...
            if (e.KeyCode == Keys.F2)
            {
                if (ug.Rows.Count > 0)
                {
                    ug.Columns[6].SortMode = DataGridViewColumnSortMode.NotSortable;
        ...
        private void button_Clicker(object sender, IdentifierButtonEventArgs e)
        {
            try
            {
                var item = Item.GetById(e.Identifier).First();
                ...
                Services.Models.SaleDetails sd = new Services.Models.SaleDetails();
                bool found = false;
        ...
        private void ug_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            var item = Item.GetById(id).First();
            ...
            var DiscountAmountIndex = ug.Columns["DiscountAmount"].Index;
            ...
            if (item.Service == 0)
            {
                ...
                {
                    if ((decimal)ug[quantityRowIndex, e.RowIndex].Value > 0)
                    {
                    ...
                    {
                        MessageBox.Show("Smundeni te vendosni vlere negative te Sasia!");
                        ug[quantityRowIndex, e.RowIndex].Value = "1";
                    }
                ...
                if (e.ColumnIndex == DiscountPrice)
                {
                    ...
                    ug[totalRowIndex, e.RowIndex].Value = totalprice;
                    ug[DiscountAmountIndex, e.RowIndex].Value = (discountPriceWithVat * quantity - totalprice).ToString("N");


                    CalculateGridColumns();
                }
                if (e.ColumnIndex == totalRowIndex)
                {
                    var t = (decimal)ug[totalRowIndex, e.RowIndex].Value;
                    var patvsh = ((Convert.ToDouble(ug["Total", e.RowIndex].Value) * (1 - Convert.ToDouble(ug["Discount", e.RowIndex].Value) / 100)) * (1 - Convert.ToDouble(clientDiscount))) * Convert.ToDouble(ug["Quantity", e.RowIndex].Value);
                    var totalsum = patvsh + (patvsh * (Convert.ToDouble(ug["VAT", e.RowIndex].Value) * 0.01));
                    if (Convert.ToDouble(t) < totalsum)
                    {
                        var d = ((Convert.ToDecimal(totalsum) - t) / (Convert.ToDecimal(totalsum))) * 100;
                        ug[DiscountPrice, e.RowIndex].Value = Math.Round(d, 2);
                        CalculateGridColumns();
                    }
                }
            }
        ...
        private void AddItem(ItemsDiscount item)
        {
            ...
            if (item.Service == 0)
            {
                if (Warehouse.GetbyId(item.Id).InStock > 0 || settings.StockWMinus != "0")
                {
                    ...
                    ((BindingList<Services.Models.SaleDetails>)ug.DataSource).Add(sd);
        ...
        private void buttonCategory_Clicker(object sender, IdentifierButtonEventArgs e)
        {
            ...
            if (mAllSubCat.Count > 0)
            {
                ...
                pnlSubCat.Height = subCbSize.Height + 10 + subCbSize.Height / 4;
        ...
        public void GenerateItems(List<SavedPOSItems> list)
        {
            var settings = Settings.Get();
            foreach (var item in list)
            {
                var items = Item.GetById(item.ItemId).First();
                var buttonNumber = items.Barcode;
                if (settings.BarcMode == 1)
                {
                    txtB.Text = buttonNumber;
                ...
                int buttonNumbers = item.ItemId;

                IdentifierButtonEventArgs args = new IdentifierButtonEventArgs(buttonNumbers);
                button_Clicker(this, args);
            }
        }
        ...
        protected void LoadSaleDetails()
        {
            ...
            List<Services.Models.SaleDetails> salesd = Services.SaleDetails.GetSaleDetailsBySaleId(mSaleId);
            BindingList<Services.Models.SaleDetails> bsalesd = new BindingList<Services.Models.SaleDetails>(salesd);
            ...
            ug.Columns[26].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            ...
            ug.Columns[4].HeaderText = "Barkodi";
            ug.Columns[5].HeaderText = "Njësia";
            ug.Columns[24].HeaderText = "Për Kthim";
            ug.Columns[26].HeaderText = "Vlera e Zbritjes";

            ug.Columns[5].Width = 50;
            ug.Columns[4].Width = 150;
        ...
        public static DataTable SaleDtoDataTables(List<Services.SaleDetails> saleDetail)
        {
            ...
            {
                DataRow r = dt.NewRow();
                var allitems = Item.GetById(saleDetails.ItemId).First();
        ...
        protected void PrintF()
        {
            ...
            if (Convert.ToDecimal(Convert.ToDecimal(txtTotalSum.Text)) == 0M && bonuscard==null)
            {
                MessageBox.Show(paragraph_fisc_with_zero.Text);
                return;
            }
            ...
            if (global.PosPrinter == "1")
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            FiscalPrinterHelper.GekosPrintOldV(sale.Id, sale.InvoiceNo, dt, mTotalSum, sale.SaleId, withBank, clientPayed);
        ...
        protected void CalculateGridColumns()
        {
            ...
            BindingList<Services.Models.SaleDetails> items = (BindingList<Services.Models.SaleDetails>)ug.DataSource;
            ...
            foreach (DataGridViewRow row in ug.Rows)
            {
                ...
                totalDiscount += Convert.ToDecimal(row.Cells[26].Value);
            }
        ...
        //{
        //    //if (Globals.CashBoxStatus != "Locked")
        //    //{
        ...
        private void ug_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
                ...
                e.RowIndex >= 0)
            {
                ...
                if (Globals.Settings.PIN == "0" || Globals.Settings.PIN == null)
                {
                    ...
                    ((BindingList<Services.Models.SaleDetails>)ug.DataSource).RemoveAt(e.RowIndex);
                    CalculateGridColumns();
                    txtTotalSum_TextChanged(null, null);
            ...
            else if (senderGrid.Columns[e.ColumnIndex].Index == ug.Columns["ForReturn"].Index && senderGrid.Columns[e.ColumnIndex] is DataGridViewCheckBoxColumn && e.RowIndex >= 0)
            {
                ...
                if ((bool)ug[24, e.RowIndex].Value == true)
                {
                    ...
                    {
                        string printertype = "";
                        if (settings.PosPrinter == "0")
                        {
                            printertype = settings.FiscalPrinterType;
                        }
                        else
                        {
                            printertype = Printer.Get().Find(p => p.Id == Globals.DeviceId).FiscalType;
                        }
                        if (printertype == "Tremol")
                        {
                ...
                {
                    var opt = MessageBox.Show($"A jeni te sigurt qe deshironi ta hiqni artikullin: {ug.Rows[e.RowIndex].Cells["ItemName"].Value.ToString()} për kthim?", "Kthim", MessageBoxButtons.YesNo);
                    if (opt.ToString() == "Yes")
                    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (DateTime.Now.ToLocalTime().ToString() == "18.04.2023 3:18:00 PM")
            {
                //ClickAll();
            }
        }
                ...
                if (cbPartners.SelectedItem != null)
                {
                    if (ug.Rows.Count > 0)
                    {
                        ...
                        if (dialogResult == DialogResult.OK)
                        {
                            ...
                            {
                                cbPartners.Text = "";
                                cbPartners.DataSource = allPartners;
                                cbPartners.Text = clients.Name;
                                cbPartners.SelectedItem = allPartners.Where(p => p.Id == clients.Id);
                                PartnerId = clients.Id;
                            }
                        }
                        else
        ...
        private void txt_TextChanged(object sender, EventArgs e)
        {
            ...
            //    try
            //    {
            //        var item = Item.GetItemWithName(txt.Text.ToLower()).ToList();
        ...
        private void AutosizeDropdown(ComboBox comboBox)
        {
            txtsearch.Cursor = Cursors.Arrow;
        ...
        private void txtSearchName_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            try
            {
                var name = txtsearch.SelectedValue != null ? Item.GetById((int)txtsearch.SelectedValue).First() : null;
                bool found = false;
        ...
        private void txtsrchB_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            //            else
            //            {
            //                var currentI = Item.GetItemWithBarcode(txtsrchB.Text);
        ...
        //Metoda per me search barcode direkt pa e fokusu inputboxin
        ...
        //    // Allow manual input with Enter:
        //    if (txtsrchB.Focused && c == (char)Keys.Enter)
        //    {
        //        _barcode = txtsrchB.Text;  // Grab the manually entered barcode
        //        txt_KeyDown(this, new KeyEventArgs(Keys.Enter));
        //        _barcode = "";  // Clear the barcode buffer
        //        return true;    // Handled the Enter key
        //    }
        //    else if (txtsrch.Focused && c == (char)Keys.Enter)
        //    {
        //        _barcode = txtsearch.Text;
        //        txtsrch_KeyPress(this, new KeyPressEventArgs('\r'));
        //        _barcode = "";
        //        return true;
        //    }
        ...
        //        _barcode += c;
        //    }
        //    else if (c == (char)Keys.Return)
        //    {
        //        if (Globals.Settings.BarcMode == 1)
        //        {
        //            txtsrchB.Text = _barcode;
        //            txt_KeyDown(this, new KeyEventArgs(Keys.Enter));
        //        }
        //        else
        //        {
        //            txtsearch.Text = _barcode;
        //            txt_KeyDown(this, new KeyEventArgs(Keys.Enter));
        //        }
        ...
        private void PosRestaurant_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            //if (settings.BarcMode == 0)
            //{
            //    if (e.KeyChar == '\r') // Check if Enter key was pressed
            //    {
            //        string barcode = barcodeInput.Trim();

            //        txtsrchB.Focus();
            //        txtsrchB.Text = barcode;

            //        barcodeInput = "";
            //    }
            //    else
            //    {
            //        // Append the entered character to the barcode input
            //        barcodeInput += e.KeyChar;
        ...
        private void txtsrch_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (txtsearch.Text != "")
            {
                ...
                if (keyPressedEventArgs != null && keyPressedEventArgs.KeyChar == (char)Keys.Enter)
                {
                    ...
                    if (settings.BarcMode == 0)
                    {
                        foundItems = mAllItems.Find(p => p.Barcode == txtsearch.Text);

                    }
                    else
                    {
                        var currentI = Item.GetItemWithBarcode(txtsearch.Text);
                        foundItems = currentI.Count > 0 ? currentI.First() : null;
        ...
        private void PosRestaurant_FormClosed(object sender, FormClosedEventArgs e)
        {


        }

        private void cbPartners_KeyDown(object sender, KeyEventArgs e)
        {
            //var text = cbPartners.Text;

            //var filterP = Partner.Search("Status=0").Where(p => p.Name == text).ToList();

            //cbPartners.DataSource = filterP;
            //cbPartners.Text = text;
        }

        private void cbPartners_KeyPress(object sender, KeyPressEventArgs e)
        {

        }

        private void SetDiscountValues()
        {
            foreach (DataGridViewRow row in ug.Rows)
            {
                row.Cells["Discount"].Value = 10; // Set the "Discount" column to 10%
            }
            ug.Refresh(); // Update the DataGridView display
        }
                ...
                try
                {
                    if (txt.Text != "Emri,Çmimi,Shifra" && txt.Text != "")
                    {
                        ...
                        if (Globals.Settings.BarcMode == 0)
                        {
                            ...
                            {
                                item = Item.GetItemWithName(txt.Text.ToLower());
