...
namespace MyNET.Pos.Modules
{
 ...
 .ToList();
        ...
        private void button17_Click(object sender, EventArgs e)
        {
            ...
            var list = new List<Sale_Archive>();
            if (comboBox1.Text != "Të gjithë")
            {

                list = Sale_Archive.SalesByDate(formattedFromDate, formattedToDate).Where(p => p.id_saler == (int)comboBox1.SelectedValue).ToList();

            }
            else
            {
                list = Sale_Archive.SalesByDate(formattedFromDate, formattedToDate);
            }
            List<Sale_Archive> sales = new List<Sale_Archive>();
            decimal t = 0.0m;
            foreach (var lsale in list)
            {
                t += lsale.TotalSum;
                sales.Add((Sale_Archive)lsale);
            ...
            dataGridView6.Columns[19].Visible = false;
            dataGridView6.Columns[20].Visible = false;
            dataGridView6.Columns[21].Visible = false;
            dataGridView6.Columns[22].Visible = false;
            dataGridView6.Columns[24].Visible = false;
            dataGridView6.Columns[25].Visible = false;
            dataGridView6.Columns[26].Visible = false;

            dataGridView6.Columns[6].HeaderText = "Nr.Faturës";
            dataGridView6.Columns[13].HeaderText = "Shuma në €";
